	...
	*/
	...
	if (!defined('ADMINISTRATION')) {
		...
		if(check_rights($_GET['action'], $_SESSION['user_ID'])) {
			...
			$result = mgb_sql_connect("SELECT id, banned_ip, matches, timestamp FROM ".$db['prefix']."banlist_ips ORDER BY ".$_GET['orderby']." ".$_GET['sort']." LIMIT $load_start, $load_end", "Error while loading banned ip entries.", 1);
