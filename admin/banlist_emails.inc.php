	...
	*/
	...
	if (!defined('ADMINISTRATION')) {
		...
		if(check_rights($_GET['action'], $_SESSION['user_ID'])) {
			...
			$result = mgb_sql_connect("SELECT id, banned_email, matches, timestamp FROM ".$db['prefix']."banlist_emails ORDER BY ".$_GET['orderby']." ".$_GET['sort']." LIMIT $load_start, $load_end", "Error while loading banned email entries.", 1);
